...
CREATE TABLE `user` (
  ...
  `updated_time` datetime NOT NULL DEFAULT '1970-01-01 00:00:00' COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `plan` (
  ...
  `name` varchar(48) NOT NULL COMMENT '推广计划名称',
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '推广计划状态',
  `start_time` datetime NOT NULL COMMENT '推广计划开始时间；',
  `end_time` datetime NOT NULL COMMENT '推广计划结束时间；',
  `created_time` datetime NOT NULL DEFAULT '1970-01-01 00:00:00' COMMENT '创建时间',
  `updated_time` datetime NOT NULL DEFAULT '1970-01-01 00:00:00' COMMENT '更新时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `unit` (
  ...
  `name` varchar(48) NOT NULL COMMENT '推广单元名称',
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '推广单元状态',
...
-- 推广单元兴趣 Feature
CREATE TABLE `unit_it` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `unit_id` int(11) NOT NULL COMMENT '推广单元 id',
  `tag` varchar(30) NOT NULL COMMENT '兴趣标签',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8 COMMENT='推广单元兴趣 Feature';


-- 推广单元地域 Feature
CREATE TABLE `unit_district` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `unit_id` int(11) NOT NULL COMMENT '推广单元 id',
